  ...
  export default {
    ...
    methods: {
      ...
      togglePlaying() {
        ...
        this.setPlayingState(!this.playing); //此处只是改变state中的playing状态，要实现播放还是要改变audio，所以要监听playing状态
        if (this.currentLyric) {  //停止或播放音乐的时候都停止或者播放歌词
      ...
      loop() {
        ...
        if (this.currentLyric) {  //循环播放时候, 当播放完一首歌，歌词重新回到第一行
      ...
      next() {
        ...
        if(this.playlist.length === 1 || this.mode === playMode.loop) {  //如果列表只有1，直接循环播放，无需执行后面操作
          this.loop();
          return;
        }else {
      ...
      updateTime(e) {
        this.currentTime = e.target.currentTime //设置或返回音频中的当前播放位置（以秒计）。
      },
      format(interval) {
      ...
      onProgressBarChange(percent) {
        ...
        if (!this.playing) {
         ...
         if (this.currentLyric) {    //当bar改变的时候，也改变歌词
      ...
      middleTouchMove(e) {
        ...
        const left = this.currentShow === 'cd' ? 0 : -window.innerWidth; //其实位置
        const offsetWidth = Math.min(0, Math.max(-window.innerWidth, left + deltaX)); //不能超出屏幕
    ...
    watch: {
      currentSong(newSong, oldSong) {
        if (!newSong.id) {   //当前歌曲列表没有歌曲之后，不执行操作
          return;
        }
        if (newSong.id === oldSong.id) {
          return;
        }
        if (this.currentLyric) {  //点击下一首或者上一首时候，清除歌词定时器
